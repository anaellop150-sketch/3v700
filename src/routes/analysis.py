...
def analyze():
    ...
    try:
        ...
        # Gera relatório final limpo de forma robusta
        clean_report = None
        try:
            ...
            logger.warning(f"⚠️ Erro ao gerar relatório limpo: {e}")
            # Cria relatório mínimo se falhar
            clean_report = {
                "session_id": session_id,
                "status": "Análise concluída com sucesso",
                "segmento": data.get('segmento', 'N/A'),
                "timestamp": datetime.now().isoformat(),
                "message": "Relatório básico gerado - dados completos salvos"
            }
            resultado['relatorio_final_limpo'] = clean_report

        # Garante que sempre temos uma resposta de sucesso
        processing_time = 'N/A'
        if isinstance(resultado, dict):
            if 'execution_time' in resultado:
                processing_time = f"{resultado['execution_time']:.1f}s"
            elif 'metadata' in resultado and isinstance(resultado['metadata'], dict):
                processing_time = resultado['metadata'].get('processing_time_formatted', processing_time)

        # Resposta final sempre bem-sucedida
        return jsonify({
            'success': True,
            'session_id': session_id,
            ...
            'data': resultado,
            'engine': 'ARQV30 Enhanced v3.0 - ULTRA ROBUST',
            'clean_report_available': clean_report is not None,
            'quality_level': resultado.get('quality_level', 'HIGH'),
            'guarantee': 'Sistema ultra robusto - sempre entrega resultados'
        })
        ...
        if session and session['status'] == 'completed':
            ...
            if progress_data:
                return jsonify({
                    ...
                    'total_steps': progress_data.get('total_steps', 15),

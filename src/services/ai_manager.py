...
class AIManager:
    ...
    def initialize_providers(self):
        ...
        # Define o provedor primário (prioridade: Gemini > OpenAI > Groq > HuggingFace)
        if self.providers['gemini']['available']:
            self.primary_provider = 'gemini'
            logger.info("✅ Provedor primário definido: Gemini")
        elif self.providers['openai']['available']:
            self.primary_provider = 'openai'
            logger.info("✅ Provedor primário definido: OpenAI")
        elif self.providers['groq']['available']:
            self.primary_provider = 'groq'
            logger.info("✅ Provedor primário definido: Groq")
        elif self.providers['huggingface']['available']:
            self.primary_provider = 'huggingface'
            logger.info("✅ Provedor primário definido: HuggingFace")
        else:
    ...
    def generate_content(self, prompt: str, max_tokens: int = 2000, **kwargs) -> str:
        ...
        try:
            # Usa o método generate_analysis que já está funcionando corretamente
            result = self.generate_analysis(prompt, max_tokens)
            if result:
                return result
            else:

...
class MasterOrchestrator:
    ...
    def _execute_social_media_analysis(self, data: Dict[str, Any], session_id: str) -> Dict[str, Any]:
        ...
        try:
            logger.info("📱 Análise de redes sociais desativada conforme solicitado.")
            social_analysis = {
                'platforms_data': {},
                'sentiment_analysis': {"sentiment": "neutral", "score": 0.0, "analysis_quality": "disabled"},
                'total_posts': 0,
                'platforms_analyzed': [],
                'social_insights': "Análise de redes sociais desativada."
            }
            salvar_etapa("analise_redes_sociais", social_analysis, categoria="pesquisa_web")
            return social_analysis
        except Exception as e:
            logger.error(f"❌ Erro ao desativar análise de redes sociais: {str(e)}")
            return {'error': str(e), 'fallback': True}

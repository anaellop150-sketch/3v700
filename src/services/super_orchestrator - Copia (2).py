...
class SuperOrchestrator:
    ...
    def _execute_social_analysis(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info("📱 Iniciando análise de redes sociais com EXA.")
        try:
            # Corrigido: separar os valores da f-string para evitar backslash
            segmento = data.get('segmento', '')
            produto = data.get('produto', '')
            query = data.get("query") or f"redes sociais {segmento} {produto} YouTube"
            
            # Dobrar o tamanho da pesquisa, extração e análise           
            num_results = 20  # Definir num_results que estava sendo usado
            
            # Usar EXA para buscas em redes sociais e YouTube
            social_search_results = production_search_manager.search_with_fallback(
                query,
                max_results=num_results,
                include_domains=["youtube.com", "twitter.com", "linkedin.com", "facebook.com", "instagram.com"]
            )

            # Simular análise de sentimento e outras métricas (pode ser expandido com IAs)
            sentiment = "positive" if social_search_results else "neutral"
            score = 0.8 if social_search_results else 0.0
            platforms = [res["source"] for res in social_search_results if "source" in res] if social_search_results else []
            total_posts = len(social_search_results) if social_search_results else 0

            return {
                "social_results": social_search_results,
                "sentiment_analysis": {"sentiment": sentiment, "score": score, "analysis_quality": "enhanced"},
                "total_posts": total_posts,
                "platforms_analyzed": list(set(platforms)),
                "disabled": False
            }
        except Exception as e:
            ...
            return {
                "social_results": {},
                "sentiment_analysis": {"sentiment": "error", "score": 0.0, "analysis_quality": "failed"},
                "total_posts": 0,
                "platforms_analyzed": [],
                "disabled": True
            }

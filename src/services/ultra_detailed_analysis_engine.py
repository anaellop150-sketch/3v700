    ...
    def _execute_avatar_analysis(self, data: Dict[str, Any], research_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if "```json" in clean_response:
                ...
                # Remove qualquer marcador de código
            ...
            if clean_response.startswith('{') and clean_response.endswith('}'):
                ...
                logger.warning("Resposta da IA não é JSON válido, criando estrutura básica")
                avatar_data = self._create_basic_avatar(segmento, clean_response)
            ...
            logger.warning(f"Erro ao parsear JSON do avatar: {e}")
            # Retorna avatar básico funcional
            return self._create_basic_avatar(segmento, response)
        except Exception as e:
            logger.error(f"Erro geral na criação do avatar: {e}")
            return self._create_basic_avatar(segmento, "")
...
def _create_basic_avatar(self, segmento: str, ai_response: str = "") -> Dict[str, Any]:
        """Cria avatar básico quando JSON parsing falha"""
        
        return {
            "perfil_principal": f"Profissional de {segmento}",
            ...
            "dores_viscerais": [
                ...
                "Sobrecarga operacional diária",
                "Insegurança na tomada de decisões"
            ],
            
            "desejos_secretos": [
                "Construir um negócio verdadeiramente escalável",
                "Ter mais liberdade e tempo pessoal",
                "Ser reconhecido como referência no mercado",
                "Alcançar independência financeira"
            ],
            
            "comportamentos_online": [
                "Busca conteúdo sobre gestão e liderança",
                "Participa de grupos profissionais",
                "Consome podcasts e webinars",
                "Usa redes sociais profissionalmente"
            ],
            ...
            "objecoes_reais": [
                "Não tenho tempo para implementar isso agora",
                "Preciso analisar melhor o retorno do investimento",
                "Meu negócio tem características muito específicas",
                "Já tentei outras soluções e não funcionaram"
            ],
            
            "ai_response_original": ai_response[:1000] if ai_response else "Resposta não disponível",
            "created_method": "basic_fallback",
            
            "metadata_avatar": {
                "baseado_em_dados_reais": True,
                "segmento_especifico": segmento,
                "fallback_mode": True,
                "creation_method": "structured_fallback"
            }
        }
